...
import {
  attachTodoBacklog,
  attachProgressToTodo,
  attachCompletedToProgress,
  attachSignToCompleted,
} from "./moveBackTask.js";
import {
  attachTodoProgress,
  attachProgressToCompleted,
  attachCompletedToSign,
  attachBacklogToTodo,
} from "./moveforwardTask.js";
import {
  attachTodoDelete,
  attachProgressDelete,
  attachCompletedDelete,
  attachBacklogDelete,
  attachSignDelete,
} from "./deleteTask.js";
...
//these variable is used for updating the task when variable's value is -1 it means we are not updating otherwise updating
 ...
 export function resetUpdateIndexes() {
...
addBtn.addEventListener("click", () => {
  ...
  addButtonTask(
    ...
    updateTodoTaskIndex,
    progressListData,
    updateprogressTaskIndex,
    updateDoneTaskIndex,
    updateBacklogTaskIndex,
    todoListData,
    completedListData,
    createTodoList,
    createProgressList,
...
export function createTodoList() {
  todoList.innerHTML = "";
  document.querySelector("#tTaskHeading").children[0].innerHTML =
    todoListData.length;
  for (let i = 0; i < todoListData.length; i++) {
       todoList.insertAdjacentHTML("beforeend", createTodoTask(todoListData[i]) );
  }
...
export function createProgressList() {
  ...
  document.querySelector("#pTaskHeading").children[0].innerHTML =
    progressListData.length;

  for (let i = 0; i < progressListData.length; i++) {
    progressList.insertAdjacentHTML(  "beforeend", createProgressTask(progressListData[i])   );
  }
  let progressListItem = document.querySelectorAll(".progressListItem");
  for (let i = 0; i < progressListData.length; i++) {
    let index = i;
    attachProgressDelete(
      progressListItem[index],
      index,
      progressListData,
      createProgressList,
      taskInput,
...
export function createCompletedList() {
  ...
  for (let i = 0; i < completedListData.length; i++) {
    completedList.insertAdjacentHTML(
      "beforeend",createCompletedTask(completedListData[i]) );
...
export function createSignList() {
  ...
  for (let i = 0; i < signListData.length; i++) {
    signList.insertAdjacentHTML( "beforeend",createSignTasks(signListData[i])     );
  }
...
export function createDelayList() {
  ...
  for (let i = 0; i < delayListData.length; i++) {
    delayList.insertAdjacentHTML("beforeend", createBacklogTask(delayListData[i]) );
  }
