...
export function createTodoList() {
  ...
  for (let i = 0; i < todoListData.length; i++) {
    ...
    attachTodoDelete(todoListItem[index], index, todoListData, createTodoList,taskInput,dueDate);
...
export function createProgressList() {
  ...
  for (let i = 0; i < progressListData.length; i++) {
    progressList.insertAdjacentHTML(
      "beforeend", createProgressTask({ title: progressListData[i].title, createDate: progressListData[i].createDate, update: progressListData[i].update, due: progressListData[i].due, })
    );
  }
  let progressListItem = document.querySelectorAll(".progressListItem");
  for (let i = 0; i < progressListData.length; i++) {
    let index = i;
    attachProgressDelete(progressListItem[index], index, progressListData, createProgressList,taskInput,dueDate);
...
export function createCompletedList() {
  ...
  document.querySelector("#dTaskHeading").children[0].innerHTML =  completedListData.length;
  
  for (let i = 0; i < completedListData.length; i++) {
    completedList.insertAdjacentHTML(
      "beforeend",createCompletedTask({ title: completedListData[i].title, createDate: completedListData[i].createDate, update: completedListData[i].update, due: completedListData[i].due,})
    );
  }
  let completedListItem = document.querySelectorAll(".completedListItem");
  for (let i = 0; i < completedListData.length; i++) {
    let index = i;
    attachCompletedDelete(completedListItem[index], index, completedListData, createCompletedList,taskInput,dueDate);
...
export function createSignList() {
  ...
  for (let i = 0; i < signListData.length; i++) {
    let index = i;
    attachSignDelete(signListItem[index], index, signListData, createSignList,taskInput,dueDate);
...
export function createDelayList() {
  ...
  for (let i = 0; i < delayListData.length; i++) {
    let index = i;
    attachBacklogDelete(delayListItem[index], index, delayListData, createDelayList,taskInput,dueDate);
